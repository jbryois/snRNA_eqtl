...
```{r,message=FALSE}
hla <- plot_eqtl(cell_type_name,gene_name,snp_name,'#55bdc2') + theme(axis.title.y = element_blank())
```
...
```{r,message=FALSE}
nsun2 <- plot_eqtl(cell_type_name,gene_name,snp_name,'#e97e72')
```
...
```{r}
met <- plot_eqtl(cell_type_name,gene_name,snp_name,'#48a8f8')
```
...
```{r}
glud1 <- plot_eqtl(cell_type_name,gene_name,snp_name,'#87ab34') + theme(axis.title.y = element_blank())
```

```{r}
p <- met + glud1 & theme(text=element_text(size=24),plot.title = element_text(size=20))
p & theme(text=element_text(size=12),plot.title = element_text(size=12))
ggsave(p,filename = 'output/figures/Figure2E.png',width=12,height=5)
```
...
coloc_heatmap('output/coloc/coloc.pd.txt',threshold=0.7,height=6,width=8.5,out_name='output/figures/Figure4D.pdf')
```

## Figure 4E

```{r,message=FALSE,fig.height=14,fig.width=10,fig.height=14}
coloc_heatmap('output/coloc/coloc.ms.txt',threshold=0.7,height=14,width=7,out_name='output/figures/Figure4E.pdf')
```
...
```{r,message=FALSE,warning=FALSE,fig.width=10}
p <- ggplot(d_sig,aes(replication_status,abs(dist+1),col=replication_status)) + 
...
p <- closest %>% 
  ...
  ylab('Number of cis-eQTL (5% FDR)') + 
  xlab('')
ggsave(p,filename='output/figures/FigureS4.png',width=5,height=3,dpi=300)
p
```
...
ggsave(p,filename='output/figures/FigureS5.png',width=6,dpi=300)
p
```

## Figure S6 - eQTL expression level
...
ggsave(p,filename='output/figures/FigureS6.png',width=14,height=5,dpi=300)
p
```

## Figure S7 - Independent eQTLs
...
ggsave(p,filename='output/figures/FigureS7B.png',width=6,height=2.5,dpi=300)
p
```
...
p <- filter(d,rank==1) %>% 
  ...
  theme(legend.position='None') +
  xlab('Distance to TSS')
ggsave(p,filename='output/figures/FigureS7C.png',width=7,dpi=300)
p
```
...
ggsave(p,filename='output/figures/FigureS7D.png',dpi=300)
p
```
...
ggsave(p,filename=paste0('output/figures/FigureS8A.png'),width=7,height=4,dpi=300)
p
```
...
ggsave(p,filename='output/figures/FigureS8B.png',width=5,height=3,dpi=300)
p
```

## Figure S9 - Epigenome enrichment
...
ggsave(p,filename = 'output/figures/FigureS9.png',width=12,height=8,limitsize = FALSE,dpi=300)
p
```

## Figure S10 - Epigenome enrichment
...
ggsave(p,filename = 'output/figures/FigureS10.png',width=8,height=6,limitsize = FALSE,dpi=300)
p
```

## Figure S11 - cell type specific eQTL
...
ggsave(p,filename='output/figures/FigureS11.png',width=12,height=5,dpi=300)
p
```

## Figure S12 - cell type specific eQTL
...
ggsave(p,filename='output/figures/FigureS12A.png',width=17,height=3,dpi=300)
p
```

```{r,message=FALSE,warning=FALSE,fig.width=12}
p <- plot_loeuf_int('At least one')
ggsave(p,filename='output/figures/FigureS12B.png',width=17,height=3,dpi=300)
p
```

```{r,message=FALSE,warning=FALSE,fig.width=12}
p <- plot_loeuf_int('All')
ggsave(p,filename='output/figures/FigureS12C.png',width=17,height=3,dpi=300)
p
```

## Figure S13 - cell type specific eQTL

### MAGMA

```{r,message=FALSE}
entrez_2_ensembl <- AnnotationDbi::toTable(org.Hs.eg.db::org.Hs.egENSEMBL) %>% 
  add_count(gene_id,name = 'n_entrez' ) %>% add_count(ensembl_id,name='n_ensembl') %>% 
  filter(n_entrez==1,n_ensembl==1) %>% 
  dplyr::select(-n_entrez,-n_ensembl) %>% 
  dplyr::rename(ensembl=ensembl_id,GENE=gene_id) %>% 
  as_tibble()
```

```{r,message=FALSE}
dir.create('data/magma_interaction_eqtl/',showWarnings = FALSE)
d <- read_tsv('output/eqtl_specific/eqtl.PC70.specific.txt') %>% 
  #Sets pvalue to NA if the model did not converge
  mutate(nb_pvalue_aggregate=ifelse(nb_pvalue_aggregate_model_converged==FALSE,NA,nb_pvalue_aggregate)) %>% 
  mutate(nb_pvalue_at_least_one=ifelse(nb_pvalue_at_least_one_model_converged==FALSE,NA,nb_pvalue_at_least_one)) %>%   #Remove genes for which the model did not converge (9 genes)
  ...
  #For each row, get the maximum pvalue across all cell types, this will be the gene-level pvalue testing whether the genetic effect on gene expression is different than all other cell types
  rowwise() %>% 
  mutate(nb_pvalue_sig_all=
           max(Astrocytes_p,`Endothelial cells_p`,`Excitatory neurons_p`,`Inhibitory neurons_p`,Microglia_p,Oligodendrocytes_p,`OPCs / COPs_p`,Pericytes_p,na.rm=TRUE)) %>% 
  ungroup() %>% 
  mutate(nb_pvalue_all_adj = p.adjust(nb_pvalue_sig_all,method='fdr')) %>% 
  separate(gene_id,into=c('symbol','ensembl'),sep='_') %>% 
  filter(nb_pvalue_all_adj<0.05) %>% 
  inner_join(.,entrez_2_ensembl,by='ensembl') %>% 
  mutate(cell_type_id=make.names(cell_type_id)) %>% 
  dplyr::select(GENE,cell_type_id) %>% 
  write_tsv(.,path = 'data/magma_interaction_eqtl/cell_type_specific.txt',col_names =FALSE)
```

```{bash,eval=TRUE,message=FALSE,results=FALSE}
cd data/magma_interaction_eqtl
source ~/.bashrc

##Gene sets
cell_type_specific_eqtl='cell_type_specific.txt'
...
##Run MAGMA
magma --gene-results $ms --set-annot $cell_type_specific_eqtl col=1,2  --out ms
magma --gene-results $ad --set-annot $cell_type_specific_eqtl col=1,2  --out ad
magma --gene-results $pd --set-annot $cell_type_specific_eqtl col=1,2  --out pd
magma --gene-results $scz --set-annot $cell_type_specific_eqtl col=1,2  --out scz
```

```{r}
files <- list.files('data/magma_interaction_eqtl',pattern='.gsa.out',full.names = T)
```

```{r,message=FALSE}
d <- tibble(file_names=files) %>% 
  mutate(trait=gsub('\\..+','',basename(file_names))) %>% 
  mutate(file_content=map(file_names,read.table,header=T)) %>%
  unnest(file_content) %>% 
  group_by(trait) %>% 
  mutate(fdr.local=p.adjust(P,method='fdr')) %>% 
  mutate(Significance=ifelse(fdr.local<=0.05,'FDR<5%','Not significant')) %>% 
  mutate(trait=factor(case_when(
    trait=='ad' ~ 'Alzheimer',
    trait=='ms' ~ 'Multiple Sclerosis',
    trait=='pd' ~ 'Parkinson',
    trait=='scz' ~ 'Schizophrenia'),levels=c('Alzheimer','Multiple Sclerosis','Parkinson','Schizophrenia'))) %>% 
  mutate(VARIABLE=gsub('\\.',' ',VARIABLE) %>% gsub('OPCs   COPs','OPCs / COPs',.))
```

```{r,message=FALSE}
cbPalette <- c("#D55E00","#56B4E9","#CC79A7")
#
p <- ggplot(d,aes(VARIABLE,-log10(P),fill=Significance)) + geom_col() + facet_wrap(~trait) + coord_flip() + theme_classic() + xlab('')
ggsave(p,filename='output/figures/FigureS13.png',width=8,dpi=300)
p
```
...
p <- plot_eqtl_specific(cell_type_name,gene_name,snp_name) + theme(text=element_text(size=20),plot.title = element_text(size=12))
p + theme(text=element_text(size=14))
ggsave(p,filename = 'output/figures/FigureS14A.png',width=9,height=5)
```
...
p <- plot_eqtl_specific(cell_type_name,gene_name,snp_name) + theme(text=element_text(size=20),plot.title = element_text(size=12))
p + theme(text=element_text(size=14))
ggsave(p,filename = 'output/figures/FigureS14B.png',width=9,height=5)
```
...
ggsave(p,filename='output/figures/FigureS15.png',width=14,height=10,dpi=300)
p
```

## Figure S16 - Number of coloc loci
...
```{r}
get_loci_explained <- function(thresh){
  filter(coloc,PP.H4.abf >= thresh) %>% 
    group_by(trait) %>% 
    summarise(n_loci_sig=length(unique(locus))) %>% 
...
```{r,message=FALSE}
thresholds <- c(0,0.7)
n_loci <- lapply(thresholds,get_loci_explained) %>% bind_rows() %>% 
  mutate(threshold=factor(case_when(
    threshold == 0 ~ 'All loci',
    threshold == 0.7 ~ 'PP >0.7',
  ),levels=c('All loci','PP >0.7')))
```

```{r,message=FALSE}
p <- n_loci %>% filter(threshold%in%c('All loci','PP >0.7')) %>% 
  ggplot(.,aes(trait,n_loci_sig,fill=threshold)) + geom_col(position = position_dodge()) + facet_wrap(~trait,scales = 'free') + theme_classic() + theme(axis.ticks.x = element_blank(),axis.text.x = element_blank(),text=element_text(size=18),legend.title = element_blank()) + xlab('') + 
  ylab('Number of loci') + geom_text(aes(label=n_loci_sig),position = position_dodge(width=0.9),vjust=1.1,size=8)
ggsave(p,filename = 'output/figures/FigureS16.png',width = 8,height = 6,dpi=300)
p
```

## Figure S17 - Exp of AD coloc genes
...
```{r}
pheatmap::pheatmap(log2(m+1),display_numbers = TRUE,number_color = 'black',angle_col = 45,cellwidth = 30,filename = 'output/figures/FigureS17.png',height = 5,width=7)
```
...
```{r,message=FALSE}
ggsave(p,filename='output/figures/FigureS18.png',width=8,height=5,dpi=300)
```

## Figure S19 - Coloc MS GTEx and DICE
...
coloc_heatmap('output/coloc/coloc.ms.gtex_dice.txt',threshold=0.7,height=30,width=10,out_name='output/figures/FigureS19.pdf')
```

## FigureS20 - Genotype PCA
...
ggsave(p,filename='output/figures/FigureS20.png',width=7,height=5,dpi=300)
p
```

## FigureS21 - eQTLs number vs PCs
...
ggsave(p,filename='output/figures/FigureS21.png',width=8,dpi=300)
p
```
