...
run_coloc <- function(tissue_sumstats,tissue_name,sumstats_locus,dice=FALSE){
  ...
  out <- lapply(unique(tissue_sumstats$gene),function(x){
    ...
    if (dice==TRUE){
      #If dice dataset, the expression data was not inverse normal transformed. We will estimate sdY from N and MAF
   ...
   direction_sign <- sumstats_locus_gene %>% 
     ...
     slice_min(n=1,p_value,with_ties=FALSE) %>% #If there are ties, take SNPs with lowest GWAS association (at random of ties).
...
coloc_results_all <- mclapply(1:nrow(loci),function(i){
  ...
  eqtl_blood_coloc <- run_coloc(eqtl_blood,'blood',sumstats_locus) %>% add_meta(type_name='eQTL')
  rm(eqtl_blood)
  
  eqtl_spleen <- prepare_eqtl_gtex(gtex_eqtl_spleen_path,chrom_locus,sumstats_locus)
  eqtl_spleen_coloc <- run_coloc(eqtl_spleen,'spleen',sumstats_locus) %>% add_meta(type_name='eQTL')
  rm(eqtl_spleen)
  
  ### sQTL
  sqtl_blood <- prepare_eqtl_gtex(gtex_sqtl_blood_path,chrom_locus,sumstats_locus)
  sqtl_blood_coloc <- run_coloc(sqtl_blood,'blood',sumstats_locus) %>% add_meta(type_name='sQTL',set_direction_to_0=TRUE)
  rm(sqtl_blood)
  
  sqtl_spleen <- prepare_eqtl_gtex(gtex_sqtl_spleen_path,chrom_locus,sumstats_locus)
  sqtl_spleen_coloc <- run_coloc(sqtl_spleen,'spleen',sumstats_locus) %>% add_meta(type_name='sQTL',set_direction_to_0=TRUE)
  rm(sqtl_spleen)
  ...
  dice_res <- lapply(1:nrow(dice_files), function(j){
    ...
    dice_coloc <- run_coloc(dice_eqtl_file,dice_files$name[j],sumstats_locus,dice=TRUE) %>% add_meta(type_name='eQTL')
    rm(dice_eqtl_file)
    return(dice_coloc)
  }) %>% bind_rows()
