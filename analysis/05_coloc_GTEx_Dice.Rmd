...
We will later add the MAF of SNPs in our study to the MS sumstats as this is required by coloc to estimate sdY when the expression data was not standard normalized (which is the case for DICE). We make the assumption that MAF in DICE is similar to the MAF in our samples.
...
```{r}
mappings <- readRDS('data/gtex/variant_id_SNP_mapping.rds')
```
...
```{r,eval=FALSE}
saveRDS(mappings,'data/gtex/variant_id_SNP_mapping.rds')
```
...
Set location of nominal result files:
...
```{r}
# gtex_eqtl_blood <- prepare_eqtl_gtex(gtex_eqtl_blood_path,chrom_locus,sumstats_locus)
prepare_eqtl_gtex <- function(gtex_tissue_path,chrom_locus,sumstats_locus){
  ...
  gtex <- read_parquet(gtex_file,
    ...
    dplyr::select(gene=phenotype_id,id=variant_id,p_eqtl=pval_nominal,beta_eqtl=slope,se_eqtl=slope_se) %>% 
    inner_join(.,mappings,by='id') %>%
    filter(variant_id%in%sumstats_locus$variant_id) %>% 
...
prepare_eqtl_dice <- function(eqtl_path,chrom_locus,sumstats_locus){
  ...
  eqtl <- fread(file,header = FALSE,data.table=FALSE) %>% 
    dplyr::select(gene=V3,p_eqtl=V7,beta_eqtl=V6,SNP_id_hg19=V2,A1=V5,A2=V4) %>% 
...
run_coloc <- function(tissue_sumstats,tissue_name,sumstats_locus,dice=FALSE){
  ...
  out <- lapply(unique(tissue_sumstats$gene),function(x){
    ...
    if (dice==TRUE){
      #If dice dataset, the expression data was not inverse transformed. We will estimate sdY from N and MAF
...
coloc_results_all <- mclapply(1:nrow(loci),function(i){
  ...
  write(i,'coloc_n_genes_done.txt',append = TRUE)
  ...
  eqtl_blood_coloc <- run_coloc(eqtl_blood,'blood',sumstats_locus) %>% mutate(type='eQTL')
  rm(eqtl_blood)
  
  eqtl_spleen <- prepare_eqtl_gtex(gtex_eqtl_spleen_path,chrom_locus,sumstats_locus)
  eqtl_spleen_coloc <- run_coloc(eqtl_spleen,'spleen',sumstats_locus)  %>% mutate(type='eQTL')
  rm(eqtl_spleen)
  
  ### sQTL
  sqtl_blood <- prepare_eqtl_gtex(gtex_sqtl_blood_path,chrom_locus,sumstats_locus)
  sqtl_blood_coloc <- run_coloc(sqtl_blood,'blood',sumstats_locus) %>% mutate(type='sQTL') %>% mutate(direction=0)
  rm(sqtl_blood)
  
  sqtl_spleen <- prepare_eqtl_gtex(gtex_sqtl_spleen_path,chrom_locus,sumstats_locus)
  sqtl_spleencoloc <- run_coloc(sqtl_spleen,'spleen',sumstats_locus) %>% mutate(type='sQTL') %>% mutate(direction=0)
  rm(sqtl_spleen)
  ...
  dice_res <- lapply(1:nrow(dice_files), function(j){
    ...
    dice_coloc <- run_coloc(dice_eqtl_B_cell_naive,dice_files$name[j],sumstats_locus,dice=TRUE) %>% mutate(type='eQTL')
    rm(dice_eqtl_file)
    return(dice_coloc)
  }) %>% bind_rows()
  
  pval_eqtl <- rbind(eqtl_blood_coloc,eqtl_spleen_coloc,sqtl_blood_coloc,sqtl_spleencoloc,dice_res)
...
```{r}
saveRDS(coloc_results_all,'coloc_gtex_dice.tmp.rds')
```
...
```{r}
coloc_results_all <- coloc_results_all %>% dplyr::select(locus:beta_top_GWAS,symbol=hgnc_symbol,ensembl=gene_parsed,nsnps:coloc_method)
```
