...
This version filters genes based on soup as well as samples. Output fastQTL format.
...
We will first compute the proportion of reads that could be attributed to ambient RNA (conservative estimate) for each sample.

```{r,eval=TRUE}
compute_ambient_per_sample <- function(id){
  message(id)
  #Get only counts for individual 'id'
  counts_id <- counts_per_cell_type %>% 
    filter(sample_id==id)
  
  #Get the cell types for which we have counts in individual 'id'
  cell_types_id <- counts_id %>% pull(cell_type) %>% unique()
  
  #Get only ambient RNA for individual 'id'
  ambient_id <- ambient %>% filter(sample_id==id) %>% 
    arrange(ensembl)
  
  #Compute ambient proportion for each gene in each cell type from individual 'id'
...
```{r,eval=TRUE}
if(file.exists('data_sensitive/ambient/ambient.prop.estimate.droplet_utils.samples.rds')){
  soup_genes <- readRDS('data_sensitive/ambient/ambient.prop.estimate.droplet_utils.genes.rds')
  soup_samples <- readRDS('data_sensitive/ambient/ambient.prop.estimate.droplet_utils.samples.rds')
} else{
  ...
  ambient_prop_all <- mclapply(samples,compute_ambient_per_sample,mc.cores = 32) %>% 
  bind_rows()
  saveRDS(ambient_prop_all,'data_sensitive/ambient/ambient.prop.estimate.droplet_utils.all.rds')

  soup_genes <- ambient_prop_all %>% 
  ...
  saveRDS(soup_genes,'data_sensitive/ambient/ambient.prop.estimate.droplet_utils.genes.rds')
  
  #Let's get gene that are not contaminated by ambient RNA.
  keep_genes <- soup_genes %>% 
    filter(mean_soup<0.1) %>% 
    dplyr::select(cell_type,ensembl)
  
  #Only keep those genes and compute the mean ambient RNA for each sample.
  soup_samples <- ambient_prop_all %>% 
    inner_join(.,keep_genes) %>% 
    group_by(individual_id,cell_type) %>% 
    summarise(mean_soup=mean(soup,na.rm=TRUE))
  
  saveRDS(soup_samples,'data_sensitive/ambient/ambient.prop.estimate.droplet_utils.samples.rds')
}
```

# Compute individual_id pseudo-bulk

We will now compute pseudo-bulk expression excluding samples for which we estimated a large fraction of ambient RNA.

First let's get sample/cell_type combination to keep

```{r}
soup_samples_keep <- soup_samples %>% 
  filter(mean_soup<0.1) %>% 
  mutate(id_celltype=paste0(individual_id,'_',cell_type))
```

```{r,eval=TRUE}
if(!file.exists('data_sensitive/expression/ms_sum_expression.individual_id.rds')){
  
  #Only keep samples/celltype with an average of less than 10% reads from ambient RNA
  sce_ms$id_celltype <- paste0(sce_ms$sample_id,'_',sce_ms$broad_annot)
  sce_ms <- sce_ms[,sce_ms$id_celltype%in%soup_samples_keep$id_celltype]
  
  #Compute Pseudo-bulk per individual
  cell_type_id <- cbind(as.character(sce_ms$broad_annot),sce_ms$individual_id) %>% 
    unique() %>%
    as.data.frame()
...
if(!file.exists('data_sensitive/expression/ad_sum_expression.individual_id.rds')){
  ...
  #Only keep samples/celltype with an average of less than 10% reads from ambient RNA
  sce_ad$id_celltype <- paste0(sce_ad$sample_id,'_',sce_ad$broad_annot)
  sce_ad <- sce_ad[,sce_ad$id_celltype%in%soup_samples_keep$id_celltype]
  
  #Compute Pseudo-bulk per individual
  cell_type_id <- cbind(as.character(sce_ad$broad_annot),sce_ad$individual_id) %>% 
    unique() %>% 
    as.data.frame()
...
```{r,eval=TRUE}
ambient_keep <- filter(soup_genes,mean_soup<0.1)
```
...
Samples with more than 10% of ambient RNA were removed above in pseudo-bulk computation.
...
get_fastqtl_pheno <- function(cell){
  ...
  bed <- sum_expression %>% dplyr::filter(cell_type==cell) %>% 
    ...
    bed <- bed %>% inner_join(.,gtf,by='gene') %>% 
      dplyr::select(`#Chr`=seqnames,start=TSS_start,end=TSS_end,ID=gene,everything()) %>% 
      arrange(`#Chr`,start)
