...
d <- read_tsv('output/eqtl/eqtl.PC60.txt')
```
...
snp_positions_file <- 'data_sensitive/eqtl/PC60_caveman/sig_eQTL_snps_alleles.txt'
```
  
```{r,eval=TRUE}
snp_coordinates <- read_tsv(snp_positions_file,col_names = FALSE) %>% setNames(c('chr','start','var_id','A1','A2'))
```
...
```{r,message=FALSE,eval=TRUE}
cov_files <- list.files('data_sensitive/eqtl/PC60/',pattern = 'cov.txt.gz',full.names = TRUE)
...
write_tsv(results,'output/eqtl_specific/eqtl.PC60.specific.txt')
```
