...
Similar as main analysis, except that soupy genes are set to NA instead (as estimating the genetic effects for a gene that is mainly driven by soup is arguably not meaningful)
...
Genes with at least 1CPM (on average) and for which the ambient RNA explain at most 10% of the reads will be considered expressed.
...
If a gene is not expressed in a cell type, we can't estimate the genetic effect on gene expression in that cell type. We set the interaction pvalue to NA for such cases.
...
interaction_model <- function(i){
  ...
  if(length(cell_type_expressed)>1){
    ...
    #If there is only one cell type, the pvalue is set to NA
    nb_pvalues_aggregate <- tibble(Estimate=NA,`Std. Error`=NA,`z value`=NA,`Pr(>|z|)`=NA)
  ...
  if(length(cell_type_expressed)>1){
    ...
    nb_pvalues <- summary(full_model_nb)$coefficients$cond %>% 
      ...
      mutate(`Pr(>|z|)`=ifelse(!variable%in%cell_type_expressed,NA,`Pr(>|z|)`)) %>% 
    ...
    nb_pvalues_all <- tibble(Astrocytes=NA,`Endothelial cells`=NA,`Excitatory neurons`=NA,
                             `Inhibitory neurons`=NA,Microglia=NA,Oligodendrocytes=NA,
                             `OPCs / COPs`=NA,Pericytes=NA)
    nb_pvalues_all[,cell_type_id] <- NA
    colnames(nb_pvalues_all) <- paste0(colnames(nb_pvalues_all),'_p')
    nb_pvalues <- tibble(p_bonf=NA)
